...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    private BarColorChangePosition mBccp;
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if( (j/4) % 2 >0) {
                if(mBccp!=null){
                   int x =  mBccp.getCCPosition();
                if(mBccp.getCCPosition()==j/8){
                    mRenderPaint.setColor(Color.parseColor("#FF7594"));
                }else{
                    mRenderPaint.setColor(Color.parseColor("#FFACBF"));
                }}
                RectF oval3 = new RectF(buffer.buffer[j], buffer.buffer[j+1], buffer.buffer[j+2], buffer.buffer[j+3]);// 设置个新的长方形
                c.drawRoundRect(oval3, 12, 12, mRenderPaint);
                c.drawRect(buffer.buffer[j], buffer.buffer[j + 1]+15, buffer.buffer[j + 2],
                        buffer.buffer[j + 3], mRenderPaint);
            }else{ if(mBccp!=null) {
                if (mBccp.getCCPosition() == j / 8) {
                    mRenderPaint.setColor(Color.parseColor("#FF7594"));
                    mRenderPaint.setAlpha(153);
                } else {
                    mRenderPaint.setColor(Color.parseColor("#FFACBF"));
                    mRenderPaint.setAlpha(153);
                }
                    ...
                    while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                        ...
                        if (vals == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (dataSet.isDrawValuesEnabled()) {
                                        drawValue(c, formatter.getBarStackedLabel(k,index,val, entry), x, y, color);
                                }
    ...
    public void setBarCCPosition(BarColorChangePosition bccp) {
        if(mBccp==null) {
            mBccp = bccp;
        }
    }
