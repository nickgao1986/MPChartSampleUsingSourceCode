...
import com.babytree.apps.pregnancy.healthanalysis.widget.barchart.XAxis.XAxisPosition;

import java.util.List;

import static com.babytree.apps.pregnancy.healthanalysis.widget.barchart.BarLineChartBase.BAR_CHART;
import static com.babytree.apps.pregnancy.healthanalysis.widget.barchart.BarLineChartBase.LINE_CHART;
import static com.babytree.apps.pregnancy.healthanalysis.widget.barchart.BarLineChartBase.LOVE_CHART;
...
public class XAxisRenderer extends AxisRenderer {
    ...
    public XAxisRenderer(ViewPortHandler viewPortHandler, XAxis xAxis, Transformer trans,int type) {
    ...
    public void renderAxisLabels(Canvas c) {
        ...
        if (mXAxis.getPosition() == XAxisPosition.TOP) {
            ...
            if(mType ==BAR_CHART) {
                drawText(c, mViewPortHandler.contentBottom() + yoffset, pointF,"月经开始日",35);
            }else if(mType==LINE_CHART||mType==LOVE_CHART) {
                drawText(c, mViewPortHandler.contentBottom() + yoffset, pointF,"日期",29);
            }
    ...
    private void drawText(Canvas c, float v, MPPointF pointF, String label, float right) {
        mAxisLabelPaint.setColor(Color.parseColor("#AAAAAA"));
        Paint.FontMetrics mFontMetricsBuffer = new Paint.FontMetrics();
        mAxisLabelPaint.getFontMetrics(mFontMetricsBuffer);
        c.drawText(label, (mViewPortHandler.contentRight())+Utils.convertDpToPixel(right), v-mFontMetricsBuffer.ascent, mAxisLabelPaint);
    }
    @Override
    public void renderAxisLine(Canvas c) {
        ...
        if (mXAxis.getPosition() == XAxisPosition.BOTTOM
                ...
                || mXAxis.getPosition() == XAxisPosition.BOTH_SIDED) {
            c.drawLine(mViewPortHandler.contentLeft(),
                    mViewPortHandler.contentBottom(), mViewPortHandler.contentRight()+Utils.convertDpToPixel(20),
...
//                String label = mXAxis.getValueFormatter().getAxisLabel((i / 2), mXAxis);
                String label = mXAxis.getValueFormatter().getAxisLabel(mXAxis.mEntries[i / 2], mXAxis,i/2);
